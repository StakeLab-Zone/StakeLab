...
if [ "$UNUNIFI_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$UNUNIFI_ADDRBOOK_URL" > "$UNUNIFI_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$UNUNIFI_HOME/data" ]
then
mkdir -p $UNUNIFI_HOME/data
# Init the chain
/bin/ununifid init --chain-id "$UNUNIFI_CHAIN_ID" "$UNUNIFI_MONIKER"


cd "$UNUNIFI_HOME/data" || exit
curl -s "$UNUNIFI_NET" > "$UNUNIFI_HOME/config/genesis.json"
if [ "$UNUNIFI_STATESYNC_ENABLE" == true ]; then
  ...
  export UNUNIFI_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export UNUNIFI_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$UNUNIFI_CHAIN_ID" == "ununifi-beta-v1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/ununifi | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$UNUNIFI_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/ununifi/snapshot.json | jq -r .latest)
  ...
  echo "Found Ununifi data folder!"
  cd "$UNUNIFI_HOME/data" || exit
fi

if [[ $UNUNIFI_DEBUG == "true" ]]; then sleep 5000; fi