...
if [ "$BITSONG_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$BITSONG_ADDRBOOK_URL" > "$BITSONG_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$BITSONG_HOME/data" ]
then
mkdir -p $BITSONG_HOME/data
# Init the chain
...
if [ "$BITSONG_STATESYNC_ENABLE" == true ]; then
  ...
  export BITSONG_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export BITSONG_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$BITSONG_CHAIN_ID" == "bitsonng-2b" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/bitsong | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$BITSONG_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/bitsong/snapshot.json | jq -r .latest)
  ...
  echo "Found Bitsong data folder!"
  cd "$BITSONG_HOME/data" || exit
fi

if [[ $BITSONG_DEBUG == "true" ]]; then sleep 5000; fi
