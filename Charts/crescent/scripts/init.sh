...
if [ "$CRESCENT_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$CRESCENT_ADDRBOOK_URL" > "$CRESCENT_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$CRESCENT_HOME/data" ]
then
mkdir -p $CRESCENT_HOME/data
# Init the chain
...
if [ "$CRESCENT_STATESYNC_ENABLE" == true ]; then
  ...
  export CRESCENT_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export CRESCENT_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$CRESCENT_CHAIN_ID" == "crescent-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/crescent | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$CRESCENT_HOME"
      ...
      *)
        ...
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/crescent/snapshot.json | jq -r .latest)
  ...
  echo "Found Crescent data folder!"
  cd "$CRESCENT_HOME/data" || exit
fi

if [[ $CRESCENT_DEBUG == "true" ]]; then sleep 5000; fi
