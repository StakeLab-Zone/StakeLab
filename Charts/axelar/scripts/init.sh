...
if [ "$AXELAR_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$AXELAR_ADDRBOOK_URL" > "$AXELAR_HOME/config/addrbook.json"
fi

# Check if Home data exists, if not create it.
if [ ! -d "$AXELAR_HOME/data" ]
then
...
if [ "$AXELAR_STATESYNC_ENABLE" == true ]; then
  ...
  export AXELAR_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export AXELAR_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  # IF MAINNET, download the latest snapshots
  if [ "$AXELAR_CHAIN_ID" == "axelar-dojo-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/axelar | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
      ...
      *)
        ...
        echo "Using latest Axelar blockchain snapshot, $SNAPSHOT_URL"
  ...
  if [ "$AXELAR_CHAIN_ID" == "axelar-testnet-lisbon-3" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/testnet-snapshots/axelar | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        echo "Using latest Polkachu blockchain snapshot, $SNAPSHOT_URL"
        aria2c --out=snapshot.tar.lz4 --summary-interval 15 --check-certificate=false --max-tries=99 --retry-wait=5 --always-resume=true --max-file-not-found=99 --conditional-get=true -s 4 -x 4 -k 1M -j 1 "$SNAPSHOT_URL"
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$AXELAR_HOME"
        rm -f snapshot.tar.lz4
  ...
  echo "Found Axelar data folder!"
  cd "$AXELAR_HOME/data" || exit
fi

if [[ $AXELAR_DEBUG == "true" ]]; then sleep 5000; fi
