...
if [ "$EVMOS_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$EVMOS_ADDRBOOK_URL" > "$EVMOS_HOME/config/addrbook.json"
fi

# Check if Home data exists, if not create it.
if [ ! -d "$EVMOS_HOME/data" ]
then
mkdir -p $EVMOS_HOME/data
# Init the chain
/bin/evmosd init --chain-id "$EVMOS_CHAIN_ID" "$EVMOS_MONIKER"

cd "$EVMOS_HOME/data" || exit
curl -s "$EVMOS_NET" > "$EVMOS_HOME/config/genesis.json"
if [ "$EVMOS_STATESYNC_ENABLE" == true ]; then
  ...
  export EVMOS_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export EVMOS_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$EVMOS_CHAIN_ID" == "evmos_9001-2" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/evmos | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$EVMOS_HOME"
      ...
      *)
        ...
        echo "Using latest Evmos blockchain snapshot, $SNAPSHOT_URL"
  ...
  echo "Found Evmos data folder!"
  cd "$EVMOS_HOME/data" || exit
fi

if [[ $EVMOS_DEBUG == "true" ]]; then sleep 5000; fi
