...
if [ "$CELESTIA_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$CELESTIA_ADDRBOOK_URL" > "$CELESTIA_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$CELESTIA_HOME/data" ]
then
...
curl -s "$CELESTIA_NET" > "$CELESTIA_HOME/config/genesis.json"
if [ "$CELESTIA_STATESYNC_ENABLE" == true ]; then
  ...
  export CELESTIA_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export CELESTIA_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$CELESTIA_CHAIN_ID" == "celestia" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/celestia | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$CELESTIA_HOME"
        rm -f snapshot.tar.lz4
        ;;
      "quicksync")
        SNAP_NAME=$(curl -s https://snaps.qubelabs.io/celestia/ | egrep -o ">celestia.*tar" | tr -d ">")
        wget -O - https://snaps.qubelabs.io/celestia/${SNAP_NAME} | tar xf - -C "$CELESTIA_HOME"/data/
        ;;
      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/celestia/snapshot.json | jq -r .latest)
  ...
  echo "Found Celestia data folder!"
  cd "$CELESTIA_HOME/data" || exit
fi

if [[ $CELESTIA_DEBUG == "true" ]]; then sleep 5000; fi