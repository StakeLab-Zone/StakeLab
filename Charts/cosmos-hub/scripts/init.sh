...
if [ "$COSMOSHUB_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$COSMOSHUB_ADDRBOOK_URL" > "$COSMOSHUB_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$COSMOSHUB_HOME/data" ]
then
mkdir -p $COSMOSHUB_HOME/data
# Init the chain
/bin/gaiad init --chain-id "$COSMOSHUB_CHAIN_ID" "$COSMOSHUB_MONIKER"

cd "$COSMOSHUB_HOME/data" || exit
curl -s "$COSMOSHUB_NET" > "$COSMOSHUB_HOME/config/genesis.json"
if [ "$COSMOSHUB_STATESYNC_ENABLE" == true ]; then
  ...
  export COSMOSHUB_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export COSMOSHUB_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$COSMOSHUB_CHAIN_ID" == "cosmoshub-4" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/cosmos | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$COSMOSHUB_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        # FIXME: add additional snapshot providers because this one is not providing snapshots for cosmoshub-4
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/cosmos/snapshot.json | jq -r .latest)
  ...
  echo "Found Cosmos Hub data folder!"
  cd "$COSMOSHUB_HOME/data" || exit
fi

if [[ $COSMOSHUB_DEBUG == "true" ]]; then sleep 5000; fi