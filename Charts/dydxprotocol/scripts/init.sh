...
if [ "$DYDX_STATESYNC_ENABLE" == true ]; then
  ...
  export DYDX_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export DYDX_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$DYDX_CHAIN_ID" == "dydx-testnet-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/dydx | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$DYDX_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/dydx/snapshot.json | jq -r .latest)
