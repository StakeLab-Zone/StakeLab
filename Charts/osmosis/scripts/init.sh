...
if [ "$OSMOSIS_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$OSMOSIS_ADDRBOOK_URL" > "$OSMOSIS_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$OSMOSIS_HOME/data" ]
then
mkdir -p $OSMOSIS_HOME/data
# Init the chain
/bin/osmosisd init --chain-id "$OSMOSIS_CHAIN_ID" "$OSMOSIS_MONIKER"


cd "$OSMOSIS_HOME/data" || exit
curl -s "$OSMOSIS_NET" > "$OSMOSIS_HOME/config/genesis.json"
if [ "$OSMOSIS_STATESYNC_ENABLE" == true ]; then
  ...
  export OSMOSIS_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export OSMOSIS_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$OSMOSIS_CHAIN_ID" == "osmosis-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/osmosis | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$OSMOSIS_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/osmosis/snapshot.json | jq -r .latest)
  ...
  echo "Found Osmosis data folder!"
  cd "$OSMOSIS_HOME/data" || exit
fi

if [[ $OSMOSIS_DEBUG == "true" ]]; then sleep 5000; fi