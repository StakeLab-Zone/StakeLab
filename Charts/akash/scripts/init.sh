...
if [ "$AKASH_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$AKASH_ADDRBOOK_URL" > "$AKASH_HOME/config/addrbook.json"
fi
...
curl -s "$AKASH_NET" > "$AKASH_HOME/config/genesis.json"
  ...
  if [ "$AKASH_CHAIN_ID" == "akashnet-2" ]; then
    ...
    case "$SNAPSHOT_PROVIDER" in
      ...
      *)
        ...
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/akash/snapshot.json | jq -r .latest)
