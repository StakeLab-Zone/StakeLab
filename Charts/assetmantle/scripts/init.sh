...
if [ "$ASSETMANTLE_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$ASSETMANTLE_ADDRBOOK_URL" > "$ASSETMANTLE_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$ASSETMANTLE_HOME/data" ]
then
mkdir -p $ASSETMANTLE_HOME/data
# Init the chain
...
if [ "$ASSETMANTLE_STATESYNC_ENABLE" == true ]; then
  ...
  export ASSETMANTLE_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export ASSETMANTLE_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$ASSETMANTLE_CHAIN_ID" == "mantle-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/ASSETMANTLE | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$ASSETMANTLE_HOME"
      ...
      *)
        ...
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/ASSETMANTLE/snapshot.json | jq -r .latest)
  ...
  echo "Found ASSETMANTLE data folder!"
  cd "$ASSETMANTLE_HOME/data" || exit
fi

if [[ $ASSETMANTLE_DEBUG == "true" ]]; then sleep 5000; fi
