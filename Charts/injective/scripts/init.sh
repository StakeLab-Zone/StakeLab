...
if [ "$INJECTIVE_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$INJECTIVE_ADDRBOOK_URL" > "$INJECTIVE_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$INJECTIVE_HOME/data" ]
then
mkdir -p $INJECTIVE_HOME/data
# Init the chain
/bin/injectived init --chain-id "$INJECTIVE_CHAIN_ID" "$INJECTIVE_MONIKER" --home "$INJECTIVE_HOME"


cd "$INJECTIVE_HOME/data" || exit
curl -s "$INJECTIVE_NET" > "$INJECTIVE_HOME/config/genesis.json"
if [ "$INJECTIVE_STATESYNC_ENABLE" == true ]; then
  ...
  export INJECTIVE_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export INJECTIVE_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$INJECTIVE_CHAIN_ID" == "injective-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/injective | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$INJECTIVE_HOME"
        rm -f snapshot.tar.lz4
        ;;
      "quicksync")
        SNAP_NAME=$(curl -s https://snaps.qubelabs.io/injective/ | egrep -o ">injective.*tar" | tr -d ">")
        wget -O - https://snaps.qubelabs.io/injective/${SNAP_NAME} | tar xf - -C "$INJECTIVE_HOME"/data/
        ;;
      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/injective/snapshot.json | jq -r .latest)
  ...
  echo "Found Injective data folder!"
  cd "$INJECTIVE_HOME/data" || exit
fi

if [[ $INJECTIVE_DEBUG == "true" ]]; then sleep 5000; fi