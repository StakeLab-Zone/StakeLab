...
if [ "$BITCANNA_ADDRBOOK_ENABLED" == true ]; then
  curl -s "$BITCANNA_ADDRBOOK_URL" > "$BITCANNA_HOME/config/addrbook.json"
fi

#Check if Home data exists, if not create it.
if [ ! -d "$BITCANNA_HOME/data" ]
then
mkdir -p $BITCANNA_HOME/data
# Init the chain
/bin/bcnad init --chain-id "$BITCANNA_CHAIN_ID" "$BITCANNA_MONIKER"

# If the node is a validator, do tx to create the validator
# Make sure the Validator has fully synced before running 
# if $BITCANNA_VALIDATOR; then
#   bcnad tx staking create-validator \
#     --from "<key-name>" \
#     --amount "1000000uhuahua" \
#     --pubkey "$(bcnad tendermint show-validator)" \
#     --chain-id "$BITCANNA_CHAIN_ID" \
#     --moniker "$BITCANNA_MONIKER" \
...
if [ "$BITCANNA_STATESYNC_ENABLE" == true ]; then
  ...
  export BITCANNA_STATESYNC_TRUST_HEIGHT=$BLOCK_HEIGHT
  export BITCANNA_STATESYNC_TRUST_HASH=$TRUST_HASH

else
  if [ "$BITCANNA_CHAIN_ID" == "bitcanna-1" ]; then
    apt -y --no-install-recommends install aria2 lz4 liblz4-tool wget > /dev/null 2>&1
    case "$SNAPSHOT_PROVIDER" in

      "polkachu")
        SNAPSHOT_URL=$(curl -s https://polkachu.com/tendermint_snapshots/bitcanna | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
        ...
        lz4 -c -d snapshot.tar.lz4 | tar -x -C "$BITCANNA_HOME"
        rm -f snapshot.tar.lz4
        ;;

      *)
        SNAPSHOT_URL=$(curl -s https://cosmos-snapshots.s3.filebase.com/bitcanna/snapshot.json | jq -r .latest)
  ...
  echo "Found Bitcanna data folder!"
  cd "$BITCANNA_HOME/data" || exit
fi

if [[ $BITCANNA_DEBUG == "true" ]]; then sleep 5000; fi